...
class EmbeddingGenerator:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2") -> None:
        ...
        log.info(event="Loading embedding model", model_name=model_name)

        # Load the sentence-transformers model
        self.model: SentenceTransformer = SentenceTransformer(model_name_or_path=model_name)

        log.info(
            event="Embedding model loaded successfully",
    ...
    def generate_embedding(self, text: str) -> np.ndarray:
        ...
        if not text or not text.strip():
            log.warning(event="Empty text provided for embedding generation")
    ...
    def generate_batch_embeddings(self, texts: list[str]) -> list[np.ndarray]:
        ...
        """
        if not texts:
            log.warning(event="Empty text list provided for batch embedding generation")
            return []
        ...
        for idx, text in enumerate[str](texts):
        ...
        embedding_dim: int = self.get_embedding_dimension()
        non_empty_idx = 0
        ...
        log.info(
            event="Batch embeddings generated successfully",
    ...
    def get_embedding_dimension(self) -> int:
        ...
        """
        dim: int | None = self.model.get_sentence_embedding_dimension()
        if dim is None:
