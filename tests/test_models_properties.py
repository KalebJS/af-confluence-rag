...
def page_strategy(draw):
    ...
    page_id = draw(st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=("Nd", "Lu", "Ll"))))
    ...
    created_date_naive = draw(st.datetimes(min_value=datetime(2000, 1, 1), max_value=datetime(2030, 12, 31)))
    created_date = created_date_naive.replace(tzinfo=timezone.utc)
    modified_date_naive = draw(st.datetimes(min_value=created_date_naive, max_value=datetime(2030, 12, 31)))
...
def document_chunk_strategy(draw):
    ...
    modified_date_naive = draw(st.datetimes(min_value=datetime(2000, 1, 1), max_value=datetime(2030, 12, 31)))
    modified_date = modified_date_naive.replace(tzinfo=timezone.utc)
