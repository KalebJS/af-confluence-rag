...
def page_strategy(draw):
    ...
    page_id = draw(st.text(min_size=1, max_size=20, alphabet=st.characters(min_codepoint=48, max_codepoint=122)))
    title = draw(st.text(min_size=1, max_size=100))
    space_key = draw(st.text(min_size=1, max_size=10, alphabet=st.characters(min_codepoint=65, max_codepoint=90)))
